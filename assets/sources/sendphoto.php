...
if(isset($_POST) && isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest'){
	...
	if($image_res){
		...
		if(normal_resize_image($image_res, $image_save_folder, $image_type, $max_image_size, $image_width, $image_height, $jpeg_quality))
		{
			...
			$time = time();	
			$mm = '<div class="post"><div class="left"><div class="profile-photo" data-src="'.$sm['user']['profile_photo'].'"></div>
					<h1>'.$sm['user']['name'].'</h1><div class="photos1"><div class="post-photo1" data-instance="photo'.$time.'" src="'.$photo.'" data-src="'.$photo.'"></div></div></div></div>';
