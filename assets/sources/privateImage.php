	...
	foreach ( $images as $key => $value){
		...
		if($image_flag){
			...
			if ($u_foto->num_rows == 0) {		
				...
				$mysqli->query("INSERT INTO users_photos(u_id,photo,thumb,approved,blocked,private)
														   VALUES ('$user','$ins_photo', '$ins_thumb','".$sm['config']['photo_review']."',1,'$private_src')");						
...
function photo($src, $dist){
	...
	if($height > $width) {
		...
		imagecopy($destino, $img, 0, 0, 78, 0, $width, $height);	
	} else {
		$destino = imagecreatetruecolor($width, $height-78);		
